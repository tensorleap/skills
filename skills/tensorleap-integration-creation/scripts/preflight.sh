...
# stops. One exception: on Copilot installs (skill folders that ship a VERSION
# file) step 0 may refresh the skill's OWN files — automatically for a global
# install, only with approval for a project-local one. Run it FIRST, before
# authoring (see SKILL.md "Preflight gate").
#
...
if [[ -f "$SKILL_DIR/VERSION" && -z "${TL_SKILL_NO_UPDATE:-}" ]] && command -v curl >/dev/null 2>&1; then
  ...
  if [[ -n "$UPDATE" && "$SKILL_DIR" == "$HOME/.copilot/skills/"* ]]; then
    ...
    if [ -t 0 ]; then
      ...
      if [[ "$ans" == [yY] ]]; then
        curl -fsSL --max-time 120 "$RAW_BASE/install.sh" | sh -s -- --tool copilot "$PROJ_ROOT" >/dev/null \
          || echo "Tensorleap skill: update failed; continuing with the installed version"
        TL_SKILL_NO_UPDATE=1 exec "$0" "$@"
      fi
    else
