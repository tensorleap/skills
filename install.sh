...
# --global (read by both VS Code and Copilot CLI); a legacy Tensorleap section
# in .github/copilot-instructions.md is removed automatically. Installs every
# skill in the repo (the Claude "plugin" grouping only matters for the native
# marketplace path, not for this file-copy install).
#
...
# Remove the marked block a pre-skill copilot install left in $1 (if any).
# Never touches content outside the markers; deletes the file only when nothing
# but whitespace remains (i.e. we created it).
remove_section() {
  target="$1"; name="$2"
  [ -f "$target" ] || return 0
  begin="<!-- BEGIN TENSORLEAP SKILL: ${name} -->"
  ...
  if ! grep -qF "$end" "$target"; then
    say "warning: $target has a BEGIN but no END marker for $name; left untouched"
    return 0
  fi
  ...
  ' "$target" > "$target.tmp"
  mv "$target.tmp" "$target"
  if ! grep -q '[^[:space:]]' "$target"; then rm -f "$target"; fi
  say "removed legacy $name section from $target"
}
...
if want copilot; then
  ...
  for skill in $(list_skills); do
    ...
    # Clean up the always-on instructions section older installers wrote.
    remove_section "$TARGET/.github/copilot-instructions.md" "$skill"
  done
fi
