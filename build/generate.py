...
def _copy_skill_assets(skill_name, skill_out):
    """Copy a skill's shared scripts/ + reference/ next to a generated SKILL.md."""
...
def emit_claude(catalog, skills, root):
    ...
    for p in catalog["plugins"]:
        ...
        for skill_name in p["skills"]:
            ...
            _copy_skill_assets(skill_name, skill_out)
...
def emit_copilot(skills, root):
    ...
    for name, canon in skills.items():
        ...
        _copy_skill_assets(name, skill_out)
