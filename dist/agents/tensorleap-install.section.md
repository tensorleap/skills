...
<!-- Tensorleap skill 'tensorleap-install' v0.1.0 — generated from skills/tensorleap-install/skill.md; do not edit here. -->
# Install Tensorleap
...
- **Never blind-retry a failed install** before matching the error in
  [install-troubleshooting.md](.tensorleap/reference/install-troubleshooting.md). Some errors are retry-safe (image-pull hiccups —
...
The failure catalog ships alongside this skill as `.tensorleap/reference/install-troubleshooting.md`;
a read-only preflight script lives at `.tensorleap/scripts/install_preflight.sh`.
...
Quick path: `bash <this skill's dir>/.tensorleap/scripts/install_preflight.sh` (use the skill's
...
- **Cloud VMs** — see the platform notes in
  [install-troubleshooting.md](.tensorleap/reference/install-troubleshooting.md#cloud-vms-ec2--sagemaker--azure): EC2 (tiny root
...
Open [install-troubleshooting.md](.tensorleap/reference/install-troubleshooting.md) and match the error text — it catalogs every
...
- `install-notes.md` survives both `uninstall` and `--purge`; only `rm -rf <data-dir>`
  destroys it.
<!-- END TENSORLEAP SKILL: tensorleap-install -->
