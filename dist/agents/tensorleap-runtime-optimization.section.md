<!-- BEGIN TENSORLEAP SKILL: tensorleap-runtime-optimization -->
<!-- Tensorleap skill 'tensorleap-runtime-optimization' v0.1.0, generated from skills/tensorleap-runtime-optimization/skill.md; do not edit here. -->
...
interpreter in `$TL_PY`). All measurements come from `.tensorleap/scripts/tl_perf.py`; you read
...
`.tensorleap/reference/perf-execution-model.md` before the first change; the short version:
...
```
poetry run python .tensorleap/scripts/tl_perf.py <subcommand> [options]
```

(Or `$TL_PY .tensorleap/scripts/tl_perf.py …`. `--root` defaults to `.`; outputs go to
...
4. **Server check (for Phase 6, non-blocking):** `.tensorleap/scripts/perf_preflight.sh`.
...
`.tensorleap/reference/perf-bottleneck-catalog.md` — for example one component calling another
...
3. CHANGE   one fix from the catalog / .tensorleap/reference/perf-levers.md. It must respect
...
Follow `.tensorleap/reference/perf-lossy-options.md`: present each option with what changes, the
...
1. `.tensorleap/scripts/perf_preflight.sh` → exit 0 required (see Phase 0 for the others).
...
`.tensorleap/reference/perf-report-template.md`, then `tl_perf report` (exit 11 → fix and
...
<!-- END TENSORLEAP SKILL: tensorleap-runtime-optimization -->
