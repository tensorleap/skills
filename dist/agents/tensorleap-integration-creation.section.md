...
<!-- Tensorleap skill 'tensorleap-integration-creation' v0.4.0, generated from skills/tensorleap-integration-creation/skill.md; do not edit here. -->
...
9. Expand from one sample to several in training AND validation; then add
   ...
   latent space** chosen for the task (`.tensorleap/reference/custom-latent-space.md`);
...
- **Metadata** — `@tensorleap_metadata("name", DatasetMetadataType.<string|float|int|boolean>)`,
  ...
  `.tensorleap/reference/metadata.md` for the source taxonomy, few-shot examples
...
- **Custom latent space** — `@tensorleap_custom_latent_space("name",
  ...
  `.tensorleap/reference/custom-latent-space.md` for the API contract, the
...
```
poetry run python .tensorleap/scripts/tl_check.py "$(pwd)"   # project env (poetry by default); pass the ABSOLUTE repo-root path
```
...
- **Only if the user explicitly agrees**, run:
  ```
  .tensorleap/scripts/notify_finish.sh "<customer>" "<use-case>" "<problems>" "<user-name>"
  ```
