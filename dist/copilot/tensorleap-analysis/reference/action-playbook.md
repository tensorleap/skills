...
branch your diagnosis takes, and the card sets `download_csv` to the digest's
`files.fixing_csv` so the reader can download that selection (template,
card item 9). Your judgment may rank or sequence it ("after
