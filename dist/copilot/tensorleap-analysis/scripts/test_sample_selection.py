...
from tl_api import sample_ids_from_csv
...
def test_root_members_only():
    ...
    ids, cols, rows = sample_ids_from_csv(blob, None, False, 10)
...
def test_no_sample_id_column():
    ids, _cols, _rows = sample_ids_from_csv(csv_bytes([AFF], [0.5]), None, False, 10)
    assert ids is None
